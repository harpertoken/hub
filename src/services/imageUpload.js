...
if (!cloudName && process.env.NODE_ENV === 'development') {
  console.warn('WARNING: No Cloudinary cloud name found in environment variables');
}
...
export const uploadPost = async (file, postData = {}, folder = 'post-media', onProgress = null) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!uploadPreset && process.env.NODE_ENV === 'development') {
        console.warn('WARNING: No Cloudinary upload preset found in environment variables');
      }
...
export const uploadImage = async (file, folder = 'post-images', onProgress = null) => {
  try {
    ...
    // Return mediaUrl (new property name) or fall back to imageUrl (old property name)
    return result.mediaUrl || result.imageUrl;
  } catch (error) {
