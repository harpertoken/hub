...
const AIFeatures = () => {
  ...
  const downloadRecording = () => {
    if (recordedVideo) {
      ...
      a.download = `ai-recording-${new Date().toISOString().slice(0, 10)}.webm`;
  ...
  const downloadMP4 = async () => {
    if (recordedBlob) {
      try {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `ai-recording-${new Date().toISOString().slice(0, 10)}.mp4`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <div className="p-2 rounded-full flex-shrink-0" style={{backgroundColor: error ? '#ef4444' : '#000000'}}>
            ...
            <div className="markdown-content ai-lab-response">
              <ReactMarkdown
                ...
                components={{
                  ...
                  ul: ({children}) => (
                    <ul className="space-y-1 mb-4 ml-4" style={{color: '#000000'}}>
                      {children}
                    </ul>
                  ),
                  ol: ({children}) => (
                    <ol className="space-y-1 mb-4 ml-4" style={{color: '#000000'}}>
                      {children}
                    </ol>
                  ),
                  li: ({children}) => (
                    <li className="text-base leading-relaxed" style={{color: '#000000'}}>
                      {children}
                    </li>
                  ),
                  strong: ({children}) => (
                    <strong className="font-semibold" style={{color: '#000000'}}>
                      {children}
                    </strong>
                  ),
                  em: ({children}) => (
