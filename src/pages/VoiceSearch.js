...
const VoiceSearch = () => {
  ...
  const isGoogleVoice = (voice) => {
    // Filter for Google voices
    return voice && voice.name && voice.name.includes('Google');
  };

  // First filter to only keep Google voices
  const googleVoices = availableVoices.filter(voice => isGoogleVoice(voice));

  // Then filter based on search term
  const filteredVoices = googleVoices.filter(voice => {
